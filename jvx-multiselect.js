...
class JvxMultiselect extends LitElement {
  render() {
    return html`
        <div id="jvxMultiselectElement" style="position:relative; display: inline; " class=${classMap({
            ...
            <div id="multiInputField" class=${classMap({
                ...
                <div class="input-container__selected-container" @click="${this._toggleMenu}">
                    <div class="input-container__selected">
                        ${this.placeholder && this.placeholder.length > 0 && (!this.label || this.label.length === 0) && this.value.length === 0 ? html`
                            <span class="placeholder">${this.placeholder}</span>` : html``}
                        ${this.multi ? html`
                                    <span>
  ...
  get jvxMultiselectElement() {
    return this.shadowRoot.querySelector('#jvxMultiselectElement');
  }
  ...
  _updateSelectionSlot() {
    let slot = this.shadowRoot.querySelector('slot[name="selected-item"]');
    if (!!slot && slot.assignedNodes().length > 0) {

      for (const item of this.value) {
        const optionTemplate = slot.assignedNodes()[0];
        const node = this.shadowRoot.querySelectorAll('.input-container__selected')[0];


        node.innerHTML = '';
        node.appendChild(optionTemplate.cloneNode(true).childNodes[0]);
        this._replaceOptionProperties(node.childNodes, item)
